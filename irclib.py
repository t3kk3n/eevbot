...
#
# This library modified by electrohead / corp[at]hush[dot]ai
...
class ServerConnection(Connection):
    ...
    def connect(self, server, port, nickname, password=None, username=None,
                ircname=None, channel=None, localaddress="", localport=0, ssl=False, ipv6=False):
        ...
        if self.password:
            self.pass_(self.password, self.server)
    ...
    def process_data(self):
        ...
        for line in lines:
            ...
            if DEBUG:
                print("Incoming server command  -  " + str(command))

            if command == "endofmotd":
                tmptarget = "NICKSERV"
                tmptext = "IDENTIFY " + self.password
                self.send_raw("PRIVMSG %s :%s" % (tmptarget, tmptext))
                self.authcheck = True

            if command == "mode":
                if (self.authcheck == True) and (arguments[1] == "+r"):
            ...
            if command == "namreply":
                tmplist = tmplist + arguments[3]

            if command == "endofnames":
                self.nicklist = tmplist
                tmplist = ""
    ...
    def list(self, channel=None, server=""):
        """Send a LIST command."""
        command = "LIST"
        if channel:
    ...
    def pass_(self, password, chkserver):
        """Send a PASS command."""
...
class SimpleIRCClient:
    ...
    def connect(self, server, port, nickname, password=None, username=None,
        ...
        This function can be called to reconnect a closed connection.
        """
        self.connection.connect(server, port, nickname, password, username, ircname, channel, localaddress, localport, ssl, ipv6)
